export function Agents() {
  ...
  return (
    <section id="agents" className="py-9">
      <div className="wrap">
        <h2 className="text-3xl font-bold mb-2.5">Agency AI agents</h2>
        <p className="text-muted mb-4">These agents are designed around the coordination and documentation burden that wrecks delivery focus.</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {agents.map((a, i) => (
            <div key={i} className="p-3.5 rounded-2xl border border-white/10 bg-white/[0.03]">
              <div className="font-extrabold mb-1">{a.h}</div>
              <div className="text-muted text-[13px]">{a.d}</div>
            </div>
          ))}
        </div>
      </div>
