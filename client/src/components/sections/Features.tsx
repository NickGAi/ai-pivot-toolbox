export function Features() {
  ...
  return (
    <section id="replace" className="py-9">
      <div className="wrap">
        <h2 className="text-3xl font-bold mb-2.5">Replace the agency tool stack</h2>
        <p className="text-muted mb-4">Keep what you love, but stop paying the “integration tax” and losing time to context switching.</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {features.map((f, i) => (
            <div key={i} className="p-3.5 rounded-2xl border border-white/10 bg-white/[0.03]">
              <div className="font-extrabold mb-1">{f.h}</div>
              <div className="text-muted text-[13px]">{f.d}</div>
            </div>
          ))}
        </div>
        ...
        <div className="card-base p-[18px]">
          <div className="font-black mb-4">Agency-native workflows included</div>
